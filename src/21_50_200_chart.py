...
def draw(block_window=BLOCK_WINDOW, log_scale=LOG_SCALE, days_back=DAYS_BACK):
    ...
    title = f'{coin_name} • 21 EMA vs 50 SMA + 200 SMA Filter + Volume + RSI(14)'
    if log_scale:
        ax1.set_yscale('log')
        title += ' (LOG)'
    if days_back:
    ...
    if SHOW_GRID: ax2.grid(True, alpha=0.3)
    ...
    # RSI
    ax3.plot(data_frame.index, data_frame['RSI'], color='#FF9900', linewidth=1.5, label='RSI(14)')
    ...
    if SHOW_GRID: ax3.grid(True, alpha=0.3)
    ...
    # Final console info
    print(f"Drawing {coin_name} chart...")
