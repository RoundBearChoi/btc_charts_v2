...
Used by 21_50_200_chart.py and resistance_and_support.py so the
menu stays in one place. Row order in coins.csv is prompt order.
"""
...
def coins_as_pairs(coins: pd.DataFrame | None = None) -> list[tuple[str, str]]:
    """Return [(display_name, symbol), ...] in menu order."""
    if coins is None:
        coins = load_coins()
...
def resolve_coin_arg(coin: str, coins: pd.DataFrame | None = None) -> tuple[str, str]:
    """Match a CLI ticker, display name, or 1-based menu index."""
    if coins is None:
        coins = load_coins()
    ...
    known = ", ".join(f"{row['name']} ({row['symbol']})" for _, row in coins.iterrows())
